...
const HomeScreen = ({ navigation }) => {
  ...
  const getAllBook = async () => {
    const data =  await requestAxios.get('/sanpham/getall')
    setBook(data.data)
    let categoriesData = [
      {
        id: 1,
  ...
  return (
    <View style={{ flex: 1, backgroundColor: COLORS.black }}>
      ...
      <ScrollView style={{ marginTop: SIZES.radius }}>
        {/* Books Section */}
        <View>{renderMyBookSection({navigation, myBooks})}</View>
        ...
        <View style={{ marginTop: SIZES.padding }}>
          ...
          <ScrollView horizontal={true} scrollEnabled={false}>
            <View>{renderCategoryData({ navigation, categories, selectedCategory })}</View>
          </ScrollView>
        </View>
      </ScrollView>
    </View>
